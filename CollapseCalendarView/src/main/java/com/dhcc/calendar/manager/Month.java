...
public class Month extends RangeUnit {
    ...
    @NonNull private HashMap<String, Object> extraData;
    private int mSelectedIndex = -1;

    protected Month(@NonNull LocalDate date, @NonNull LocalDate today, @Nullable LocalDate minDate,
                    @Nullable LocalDate maxDate,@Nullable HashMap<String, Object> extraData) {
        super(
	...
	public boolean today() {
		 setFrom(getToday().withDayOfMonth(1));
         setTo(getToday().withDayOfMonth(getFrom().dayOfMonth().getMaximumValue()));
         build();
         return true;
	}
    ...
    public void build() {
        ...
        for (int i = 0; i == 0 || getTo().compareTo(date) >= 0; i++) {
            mWeeks.add(new Week(date, getToday(), getMinDate(), getMaxDate(),extraData));
