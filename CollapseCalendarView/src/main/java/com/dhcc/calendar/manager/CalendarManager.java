package com.dhcc.calendar.manager;
...
public class CalendarManager {
    ...
    @NonNull private LocalDate mDisplay;
    ...
    @NonNull private HashMap<String, Object> extraData = new HashMap<String, Object>();
    ...
    public void setDisplayDay(@NonNull LocalDate date) {
    	mDisplay=date;
    }
    ...
    public void refreshView() {
    	if(mState == State.MONTH) {
    		setUnit(new Month(mDisplay, mToday, mMinDate, mMaxDate,getExtraData()));
            mUnit.select(mSelected);
    	} else {
    		setUnit(new Week(mDisplay, mToday, mMinDate, mMaxDate,getExtraData()));
            mUnit.select(mSelected);
    	}
    ...
    private void toggleFromMonth(LocalDate date) {
        setUnit(new Week(date, mToday, mMinDate, mMaxDate,getExtraData()));
        mUnit.select(mSelected);
        mState = State.WEEK;
    }
    ...
    private void toggleFromWeek() {
        ...
        setUnit(new Month(mActiveMonth, mToday, mMinDate, mMaxDate,getExtraData()));
        mUnit.select(mSelected);
    ...
    private void init() {
        if (mState == State.MONTH) {
            setUnit(new Month(mSelected, mToday, mMinDate, mMaxDate,getExtraData()));
        } else {
            setUnit(new Week(mSelected, mToday, mMinDate, mMaxDate,getExtraData()));
        }
        mUnit.select(mSelected);
    }
